...
extension WheelView {
  func performSelection() {
    ...
    let selection = animator.selectionSpin(delegate: self, duration: selectionSpinDuration, selectionAngle: selectionAngle)
...
extension WheelView {
  ...
  func clackyClack(speed: Speed, duration: CFTimeInterval) -> Timer {
    ...
    return Timer.scheduledTimer(timeInterval: rate, target: self, selector: #selector(claks), userInfo: nil, repeats: true)
  }
